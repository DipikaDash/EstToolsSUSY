...
const TString inputdir = "root://cmseos.fnal.gov//eos/uscms/store/user/ddash/nanoAOD/ana_samplesv1/";
...
const TString lumistr = "21.1"; //Units are in pb
...
//const TString wgtvar = lumistr+"*1000*Stop0l_evtWeight*Stop0l_trigger_eff_MET_loose_baseline*puWeight*BTagWeight*ISRWeight*PrefireWeight";
  const TString wgtvar = lumistr+"*1000*Stop0l_evtWeight*puWeight*BTagWeight";//*ISRWeight";
...
bool isValidate = false; //Adding bool to add dphiCut > 0.7
...
const TString trigDiLepCR = " && passtrigdilepOR && dileppt>200";
  const TString datasel = "";//&& (Pass_EventFilter || run==1)  ";//&& Jet_pt[0] >30 && Jet_pt[1] >30";// && JetPass_pt[0] >30 && JetPass_pt[1] >30";// && Pass_HT && Pass_dPhiMETLowDM";
//const TString datasel = " && Pass_EventFilter && Pass_HT && Pass_dPhiMETLowDM && Pass_HEMVeto20";
  const TString qcdSpikeRemovals = "";// && (!(luminosityBlock==40062 && event==91000735))";
//const TString dphi_invert = " && (Jet_dPhiMET[0]<0.1 || Jet_dPhiMET[1]<0.1 || Jet_dPhiMET[2]<0.1)";
  const TString dphi_invert = " && Pass_QCDCR_highDM && Pass_NJets20 && Pass_CaloMETRatio";// && Pass_JetID";// && Pass_j302p2 && Jet_chHEF[0]>0.05 &&  Jet_neEmEF[0]<0.7 && Jet_neHEF[0]<0.8";// &&(!Pass_j202p2)";//&&Pass_JetID && (PV_npvs>10 && PV_npvs <28) ";//Pass_JetID&& Jet_chHEF[0]>0.05 && Jet_neEmEF[0]<0.7 && Jet_neHEF[0]<0.8";//&& Pass_exHEMVeto30";// && Jet_neEmEF<0.7";//&& Pass_exHEMVeto30 ";//&& (PV_npvs>10 && PV_npvs <28)";"; 
...
BaseConfig qcdConfig(){
  ...
  // qcdcr
  config.addSample("data-cr",     "Data",              "data_2018AB",  "1.0",      datasel + trigSR + vetoes + dphi_invert);
  config.addSample("qcd-cr",      "QCD",              "qcd_orig_2018",       qcdwgt,      datasel + trigSR + dphi_invert + qcdSpikeRemovals);

  config.addSample("qcd-withveto-cr",  "QCD",         "qcd_orig_2018",       qcdvetowgt,  datasel + trigSR + vetoes + dphi_invert + qcdSpikeRemovals);
  config.addSample("qcd-withveto-sr",  "QCD",         "qcd_orig_2018",       qcdvetowgt,  datasel + trigSR + vetoes + qcdSpikeRemovals);

  // qcdcr: other bkg subtraction
  config.addSample("ttbar-cr",       "t#bar{t}",      "ttbar_2018",        lepvetowgt,  datasel + trigSR + vetoes + dphi_invert);
  config.addSample("wjets-cr",       "W+jets",        "wjets_2018",        lepvetowgt,  datasel + trigSR + vetoes + dphi_invert);
  config.addSample("minor-cr",          "minor",            "minor_2018",           lepvetowgt,  datasel + trigSR + vetoes + dphi_invert);
  config.addSample("rare-cr",         "rare",           "rare_2018",          lepvetowgt,  datasel + trigSR + vetoes + dphi_invert);
  config.addSample("znunu-cr",       "Znunu",         "znunu_2018",        lepvetowgt,  datasel + trigSR + vetoes + dphi_invert);

  // onelepcr: norm correction for other bkg subtraction
  config.addSample("data-norm",      "Data",          "data_2018AB", "1.0",       datasel + trigSR + revert_vetoes + dphi_cut);
  config.addSample("ttbar-norm",     "t#bar{t}",      "ttbar_2018",        lepselwgt,   datasel + trigSR + revert_vetoes + dphi_cut);
  config.addSample("wjets-norm",     "W+jets",        "wjets_2018",        lepselwgt,   datasel + trigSR + revert_vetoes + dphi_cut);
  config.addSample("tW-norm",        "tW",            "minor_2018",           lepselwgt,   datasel + trigSR + revert_vetoes + dphi_cut);
  config.addSample("ttW-norm",       "ttW",           "rare_2018",          lepselwgt,   datasel + trigSR + revert_vetoes + dphi_cut);
  config.addSample("qcd-norm",       "QCD",           "qcd_orig_2018",       lepselwgt,   datasel + trigSR + revert_vetoes + dphi_cut + qcdSpikeRemovals);

  // qcdsr
  config.addSample("qcd-sr",         "QCD",           "qcd_orig_2018",       qcdwgt,      datasel + trigSR + qcdSpikeRemovals);

  // samples for splitting the TF, optional (see z.splitTF)
  if (SPLITTF){
    config.addSample("qcd-sr-int",     "QCD",           "qcd_orig_2018",       qcdwgt,      datasel + trigSR + dphi_cut + qcdSpikeRemovals);
  }
